...
const AddTripForm = ({ newLocation, onClose, onSaveSuccess }) => {
  ...
  // Reset form when closing or opening new location
  useEffect(() => {
    if(!newLocation) {
  ...
  // --- NEW: Handler to remove a photo before upload ---
  const removePhoto = (indexToRemove) => {
    // 1. Create copy of array
    const updatedPhotos = [...photos];
    // 2. Revoke object URL to free memory (good practice)
    URL.revokeObjectURL(updatedPhotos[indexToRemove].previewUrl);
    // 3. Remove item at index
    ...
    try {
      ...
      if (!isBucketList && photos.length > 0) {
          const imagePromises = photos.map(async (photo) => {
            ...
            // Important: Revoke the local preview URL after successful upload
  ...
  const displayAddress = newLocation?.address 
    ? newLocation.address 
  ...
  return (
    <div className={`sidebar-panel add-sidebar ${newLocation ? 'open' : ''}`}>
      ...
      <div className="form-container">
        ...
        {newLocation && (
            <div style={{
                background: '#e3f2fd', padding: '15px', borderRadius: '16px', marginBottom: '25px', 
                color: '#0d47a1', display:'flex', flexDirection:'column', gap: '6px', border: '1px solid #bbdefb'
            }}>
                <div style={{display:'flex', alignItems:'center', gap:'8px', fontWeight:'700', fontSize:'0.9rem'}}>
                    <FaMapMarkerAlt /> Selected Location
                </div>
                <div style={{
                    lineHeight: 1.5, opacity: 0.9, fontSize:'0.95rem', fontWeight:'500',
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: '16px' }}>
                ...
                <div className="form-group" style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '5px', marginTop:'10px' }}>
                        {[...Array(5)].map((_, i) => (
                            <FaStar key={i} size={20} cursor="pointer" color={(i + 1) <= formData.rating ? "#FF9F1C" : "#E2E8F0"} onClick={() => setFormData({ ...formData, rating: i + 1 })} />
                        ))}
                    </div>
                </div>
              </div>
          ...
          {!isBucketList && (
              <div className="form-group">
                ...
                {/* --- UPDATED PHOTO PREVIEW LIST --- */}
                {photos.map((p, index) => (
                  <div key={index} className="preview-card">
                    {/* Image Container for positioning the delete button */}
                    <div style={{position: 'relative', width: '50px', height: '50px', flexShrink: 0}}>
                        ...
                        <button 
                            ...
                            style={{
                                ...
                                background: '#ff4757', color: 'white', border: '2px solid white',
